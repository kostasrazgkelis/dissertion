...
kubectl wait --for=condition=available --timeout=600s deployment/django-backend -n spark-cluster

# Get the Flask container name and namespace
$FLASK_POD = kubectl get pods -l app=django-backend -o jsonpath="{.items[0].metadata.name}" -n spark-cluster
...
kubectl port-forward $FLASK_POD 8000:8080 -n spark-cluster
#
